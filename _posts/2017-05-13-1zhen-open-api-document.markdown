...
|count|每页的问诊数|Integer|11|是|否|分页查询里，每一页包含的问诊数，最大50|
|start_num|页码|Integer|11|是|否|分页查询里，要查询的页数。例如：第一页传1，第二页传2|
...
| 字段 | 说明 | 类型 | 长度 | 必要 |备注|
| :-- | :-- | :-- | -- | -- | :-- |
|ret|接口调用返回码|Integer|11|是|1: 成功, 其他: 错误码|
|msg|接口调用结果消息|String|256|是|如果调用失败，这里有错误信息|
|data|数据结果|Object|/|是|数据定义，参考"[问诊结果](#inquery_result)"|

* 问诊结果<a name="inquery_result"/>

| 字段 | 说明 | 类型 | 长度 | 必要 |备注|
| :-- | :-- | :-- | -- | -- | :-- |
|ret|接口调用返回码|Integer|11|是|1: 成功, 其他: 错误码|
|partner_inquery_list|诊单数组|Array(Object)|/|是|诊单数组，诊单对象定义，参考"[诊单对象](#inquery_data)"|
